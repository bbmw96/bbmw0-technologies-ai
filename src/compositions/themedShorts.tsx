// Six 40-second themed Shorts. Each picks a theme + a list of beats + an
// audio file. 40s = 1200 frames @ 30fps.
//
// Sound files live in public/sounds/. staticFile() URL-encodes spaces, so
// filenames with spaces work fine ("ocean waves.mp3" becomes "ocean%20waves.mp3").
...
const consensusBeats = (url: string): Beat[] => [
  { kind: "title", eyebrow: "Five brains", text: "One prompt.", sub: "Five different answers.", durationInFrames: 150 },
  { kind: "trio", words: ["Claude.", "GPT.", "Gemini."], durationInFrames: 150 },
  { kind: "trio", words: ["Perplexity.", "Llama.", "All five."], durationInFrames: 150 },
  { kind: "bigword", text: "Best of each.", durationInFrames: 180 },
  { kind: "list", heading: "Consensus mode", items: ["Asks every model in parallel", "Merges by majority vote", "One sharper answer"], durationInFrames: 270 },
  { kind: "stat", number: "5x", label: "the perspective. One result.", durationInFrames: 150 },
  { kind: "cta", headline: "Try it.", url, durationInFrames: 150 },
];

export const ConsensusShort: React.FC<ShortProps> = ({ url = URL, audioUrl = "sounds/Drums.mp3" }) => (
  <ThemedShort themeId="sunset" beats={consensusBeats(url)} audioUrl={audioUrl} audioVolume={0.4} />
);
export const consensusShortDefaults: ShortProps = { url: URL, audioUrl: "sounds/Drums.mp3" };
...
const phoneInstallBeats = (url: string): Beat[] => [
  ...
  { kind: "list", heading: "Add to home screen", items: ["iPhone: Safari to Share to Add", "Android: Chrome to menu to Install", "Done"], durationInFrames: 300 },
...
export const PhoneInstallShort: React.FC<ShortProps> = ({ url = URL, audioUrl = "sounds/Drums.mp3" }) => (
  <ThemedShort themeId="midnight" beats={phoneInstallBeats(url)} audioUrl={audioUrl} audioVolume={0.4} />
);
export const phoneInstallShortDefaults: ShortProps = { url: URL, audioUrl: "sounds/Drums.mp3" };
...
const languagesBeats = (url: string): Beat[] => [
  ...
  { kind: "list", heading: "On launch", items: ["English, Espanol, Francais, Deutsch", "Portugues, Japanese, Chinese, Arabic", "Hindi, Russian"], durationInFrames: 300 },
...
export const LanguagesShort: React.FC<ShortProps> = ({ url = URL, audioUrl = "sounds/ocean waves.mp3" }) => (
  <ThemedShort themeId="ocean" beats={languagesBeats(url)} audioUrl={audioUrl} audioVolume={0.5} />
);
export const languagesShortDefaults: ShortProps = { url: URL, audioUrl: "sounds/ocean waves.mp3" };
...
export const PresetsShort: React.FC<ShortProps> = ({ url = URL, audioUrl = "sounds/Drums.mp3" }) => (
  <ThemedShort themeId="sandstorm" beats={presetsBeats(url)} audioUrl={audioUrl} audioVolume={0.4} />
);
export const presetsShortDefaults: ShortProps = { url: URL, audioUrl: "sounds/Drums.mp3" };
...
const openSourceBeats = (url: string): Beat[] => [
  ...
  { kind: "stat", number: "GBP 0", label: "to use, fork, ship, sell.", durationInFrames: 180 },
...
export const OpenSourceShort: React.FC<ShortProps> = ({ url = URL, audioUrl = "sounds/forest ambiance.mp3" }) => (
  <ThemedShort themeId="forest" beats={openSourceBeats(url)} audioUrl={audioUrl} audioVolume={0.5} />
);
export const openSourceShortDefaults: ShortProps = { url: URL, audioUrl: "sounds/forest ambiance.mp3" };
...
const mobileFirstBeats = (url: string): Beat[] => [
  ...
  { kind: "list", heading: "Design rules", items: ["Tap targets at least 44pt", "One thumb operation", "No hover, no menus"], durationInFrames: 270 },
...
export const MobileFirstShort: React.FC<ShortProps> = ({ url = URL, audioUrl = "sounds/thunder and rain.mp3" }) => (
